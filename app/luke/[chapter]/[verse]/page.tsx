...
import { getLukeVerseData, getLukeDefaultVerseData, getLukeChapterData } from '@/lib/data'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = getLukeVerseData(c, v) || getLukeDefaultVerseData(c, v)
  return {
    title: `Luke ${c}:${v} — OpenScripture`,
    description: data.plain_english_summary.slice(0, 160),
  }
}

export default async function LukeVersePage({ params }: Props) {
  const { chapter, verse } = await params
  ...
  const verseData = getLukeVerseData(chapterNum, verseNum) || getLukeDefaultVerseData(chapterNum, verseNum)
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <nav className="flex items-center gap-2 text-sm text-zinc-400 mb-8">
        ...
        <span>/</span>
        <Link href={`/luke/${chapterNum}`} className="hover:text-zinc-600 transition-colors">Chapter {chapterNum}</Link>
        <span>/</span>
      ...
      <div className="mt-12 flex items-center justify-between border-t border-zinc-200 pt-8">
        {prevVerse ? (
          <Link
            href={`/luke/${chapterNum}/${prevVerse}`}
            className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors"
          >
            ← Luke {chapterNum}:{prevVerse}
          </Link>
        ) : (
          chapterNum > 1 ? (
            <Link href={`/luke/${chapterNum - 1}`} className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors">
              ← Chapter {chapterNum - 1}
            </Link>
          ) : <div />
        )}

        <Link href={`/luke/${chapterNum}`} className="text-sm text-zinc-400 hover:text-zinc-600 transition-colors">
          Chapter {chapterNum}
        </Link>

        {nextVerse ? (
          <Link
            href={`/luke/${chapterNum}/${nextVerse}`}
            className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors"
          >
            Luke {chapterNum}:{nextVerse} →
          </Link>
        ) : (
          chapterNum < 24 ? (
            <Link href={`/luke/${chapterNum + 1}`} className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors">
