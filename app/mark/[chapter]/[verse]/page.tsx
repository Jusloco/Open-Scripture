...
import { getMarkVerseData, getMarkDefaultVerseData, getMarkChapterData } from '@/lib/data'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = getMarkVerseData(c, v) || getMarkDefaultVerseData(c, v)
  return {
    title: `Mark ${c}:${v} — OpenScripture`,
    description: data.plain_english_summary.slice(0, 160),
  }
}

export default async function MarkVersePage({ params }: Props) {
  const { chapter, verse } = await params
  ...
  const chapterData = getMarkChapterData(chapterNum)
  if (!chapterData) notFound()
  if (verseNum < 1 || verseNum > chapterData.verse_count) notFound()

  const verseData = getMarkVerseData(chapterNum, verseNum) || getMarkDefaultVerseData(chapterNum, verseNum)
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <nav className="flex items-center gap-2 text-sm text-zinc-400 mb-8">
        <Link href="/mark" className="hover:text-zinc-600 transition-colors">Mark</Link>
        <span>/</span>
        <Link href={`/mark/${chapterNum}`} className="hover:text-zinc-600 transition-colors">Chapter {chapterNum}</Link>
        <span>/</span>
      ...
      {/* Manuscript Reader — image + word-by-word comparison */}
      {verseData.greek_words && verseData.greek_words.length > 0 && (
        <ManuscriptReader
          wordPairs={verseData.greek_words}
          manuscript={verseData.earliest_manuscript}
        />
      )}
      ...
      <div className="mt-12 flex items-center justify-between border-t border-zinc-200 pt-8">
        {prevVerse ? (
          <Link
            href={`/mark/${chapterNum}/${prevVerse}`}
            className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors"
          >
            ← Mark {chapterNum}:{prevVerse}
          </Link>
        ) : (
          chapterNum > 1 ? (
            <Link href={`/mark/${chapterNum - 1}`} className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors">
              ← Chapter {chapterNum - 1}
            </Link>
          ) : <div />
        )}

        <Link href={`/mark/${chapterNum}`} className="text-sm text-zinc-400 hover:text-zinc-600 transition-colors">
          Chapter {chapterNum}
        </Link>

        {nextVerse ? (
          <Link
            href={`/mark/${chapterNum}/${nextVerse}`}
            className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors"
          >
            Mark {chapterNum}:{nextVerse} →
          </Link>
        ) : (
          chapterNum < 16 ? (
            <Link href={`/mark/${chapterNum + 1}`} className="text-sm text-zinc-500 hover:text-zinc-700 transition-colors">
