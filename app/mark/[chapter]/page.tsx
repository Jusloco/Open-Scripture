...
import { getMarkChapterData, getMarkVersesByChapter, getMarkBookData } from '@/lib/data'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const data = getMarkChapterData(chapterNum)
  if (!data) return { title: 'Chapter Not Found' }
  return {
    title: `Mark Chapter ${chapterNum}`,
    description: data.summary,
  }
}

export default async function MarkChapterPage({ params }: Props) {
  const { chapter } = await params
  const chapterNum = parseInt(chapter)
  ...
  const keyVerses = getMarkVersesByChapter(chapterNum)

  const prevChapter = chapterNum > 1 ? chapterNum - 1 : null
  const nextChapter = chapterNum < 16 ? chapterNum + 1 : null
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-16">
      ...
      {/* Header */}
      <div className="mb-10">
        <h1 className="text-3xl font-bold text-zinc-900 mb-2">Mark — Chapter {chapterNum}</h1>
              ...
              <div className="flex items-center gap-4">
                ...
                {isDocumented && verseData ? (
                  <div>
                    ...
                    {!verseData.text && (
                      <p className="text-sm text-zinc-500">Mark {chapterNum}:{verseNum}</p>
                    )}
                  </div>
                ) : (
                  <p className="text-sm text-zinc-400">Mark {chapterNum}:{verseNum}</p>
                )}
              </div>
      ...
      <div className="mt-12 flex items-center justify-between border-t border-zinc-200 pt-8">
        ...
        <Link href="/mark" className="text-sm text-zinc-400 hover:text-zinc-600 transition-colors">
          All Chapters
        </Link>

        {nextChapter ? (
          <Link
            href={`/mark/${nextChapter}`}
            className="flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-700 transition-colors"
          >
            Chapter {nextChapter} →
          </Link>
